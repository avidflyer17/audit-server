...
describe('services UI', () => {
  ...
  test('service details are visible by default', () => {
    ServiceStore.setData(['sshd']);
    initServicesUI();
    renderServicesList();
    const item = document.querySelector('#servicesList .service-item');
    expect(item.hasAttribute('aria-expanded')).toBe(false);
    expect(item.querySelector('.service-details')).not.toBeNull();
  });
});
